using CatViews.CatView, NLSolversBase, NLsolve, Optim, Distributions, Juno, Gadfly, DifferentialEquations, ArgParse, Distributions, Plots
...
@views function gradient_covariance_check_obs_m!(
    ...
    # p_stack = vcat(p_stack,
    # Gadfly.plot(
    # layer(x=minres.minimizer[1:1], y=minres.minimizer[2:2], Geom.point, Theme(default_color=color("blue"))),
    # layer(x=p[1:1], y=p[2:2], Geom.point, Theme(default_color=color("orange"))),
    # layer(z=(x,y) -> (initialize_p!(a, [x, y]); orbit_negative_log_likelihood!(a, model, pseudo_obs, pseudo_obs_var)), x=linspace(1e-5, max(initial_upper_bounds[1], 2.*minres.minimizer[1]), 100), y=linspace(1e-5, max(initial_upper_bounds[2], 2.*minres.minimizer[2]), 100), Geom.contour(levels=100)),
    # white_panel))
...
Base.@ccallable function julia_main(args::Vector{String})::Cint
    ...
    @add_arg_table settings begin
        ...
        "--pseudo-obs-var"
            ...
            nargs = '+'
            # default = [1., 1., 1., 1., 1.]
            default = [0.000001, 0.000001, 0.000001, 0.000001, 0.000001]
        "--obs-variance"
