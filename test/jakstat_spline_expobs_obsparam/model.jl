...
function observation!(m::Model, t, x, p)
    ex = exp.(x)
    m.observation[1] = p[10] + p[11] * (        ex[2] + 2ex[3])
    m.observation[2] = p[12] + p[13] * (ex[1] + ex[2] + 2ex[3])
...
function observation_jacobianx!(m::Model, t, x, p)
    ex = exp.(x)
    m.observation_jacobianx[1,2] =  p[11] * ex[2]
    m.observation_jacobianx[1,3] = 2p[11] * ex[3]

    m.observation_jacobianx[2,1] =  p[13] * ex[1]
    m.observation_jacobianx[2,2] =  p[13] * ex[2]
    m.observation_jacobianx[2,3] = 2p[13] * ex[3]
    nothing
end
...
function observation_jacobianr!(m::Model, t, x, p)
    ...
    m.observation_jacobianr[1,2] =         ex[2] + 2ex[3]

    m.observation_jacobianr[2,3] = 1.
    m.observation_jacobianr[2,4] = ex[1] + ex[2] + 2ex[3]
    nothing
end

function observation_hessianxx!(m::Model, t, x, p)
    ex = exp.(x)
    m.observation_hessianxx[1,2,2] =  p[11] * ex[2]
    m.observation_hessianxx[1,3,3] = 2p[11] * ex[3]

    m.observation_hessianxx[2,1,1] =  p[13] * ex[1]
    m.observation_hessianxx[2,2,2] =  p[13] * ex[2]
    m.observation_hessianxx[2,3,3] = 2p[13] * ex[3]
    nothing
end

function observation_hessianxr!(m::Model, t, x, p)
    ex = exp.(x)
    m.observation_hessianxr[1,2,2] =  ex[2]
    m.observation_hessianxr[1,3,2] = 2ex[3]

    m.observation_hessianxr[2,1,4] =  ex[1]
    m.observation_hessianxr[2,2,4] =  ex[2]
    m.observation_hessianxr[2,3,4] = 2ex[3]
    nothing
end
