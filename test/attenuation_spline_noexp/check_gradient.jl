...
include("../../src/adjoint_backward_spline_attenuation_noexp.jl")
include("../../src/assimilate_backward.jl")

include("../../util/check_args.jl")
include("../../util/check_gradient_backward_spline_attenuation.jl")
include("model.jl")
...
@views function gradient_covariance_check_obs_m!(
    ...
    for _replicate in 1:replicates
        for _l in 1:length(time_point)
            ...
            obs[N+1, Int(time_point[_l]/dt) + 1, _replicate] = a.p[_l+1] + rand(d[N+1])
        end
    end
    ...
    if !isnull(res_ana.precision) && !isnull(res_num.precision)
        ...
        println("prec_num[:,3]")
        Base.print_matrix(STDOUT, prec_num[:,3])
