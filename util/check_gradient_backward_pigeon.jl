...
function plot_twin_experiment_result_wo_errorbar_observation(dir, a::Adjoint{N,L,R,U,K}, m::Model, tob, obs, true_params) where {N,L,R,U,K}
    ...
    for _i in 1:U
        df_tob = DataFrame(t=t, x=[(m.observation!(m, t[_j], tob[:,_j], true_params); m.observation[_i]) for _j in 1:a.steps+1], data_type="true orbit")
        _mask = isfinite.(view(reshape(obs, U, :), 1, :))
        df_obs = DataFrame(t=view(repeat(t; outer=[K]), :)[_mask], x=view(reshape(obs, U, :), _i, :)[_mask], data_type="observed")
        df_assim = DataFrame(t=t, x=[(m.observation!(m, t[_j], a.x[:,_j], a.p); m.observation[_i]) for _j in 1:a.steps+1], data_type="assimilated")
        p_stack = vcat(p_stack,
        Gadfly.plot(
...
function plot_position(dir, a::Adjoint{N,L,R,U,K}, m::Model, tob, obs) where {N,L,R,U,K}
    white_panel = Theme(panel_fill="white")
    p_stack = Array{Gadfly.Plot}(undef, 0)
    t = collect(0.:a.dt:a.steps*a.dt)
    df_tob1 = DataFrame(x=a.x[1,:], y=a.x[2,:], data_type="true orbit")
    df_obs1 = DataFrame(x=obs[1,:,1], y=obs[2,:,1], data_type="observed")
    df_tob2 = DataFrame(x=a.x[5,:], y=a.x[6,:], data_type="true orbit")
    df_obs2 = DataFrame(x=obs[3,:,1], y=obs[4,:,1], data_type="observed")

    p_stack = vcat(p_stack,
    Gadfly.plot(
    layer(df_tob1, x="x", y="y", color=:data_type, Geom.line, Theme(default_color=color("blue"))),
    layer(df_obs1, x="x", y="y", color=:data_type, Geom.point, Theme(default_color=color("orange"))),
    layer(df_tob2, x="x", y="y", color=:data_type, Geom.line, Theme(default_color=color("green"))),
    layer(df_obs2, x="x", y="y", color=:data_type, Geom.point, Theme(default_color=color("grey"))),
    Guide.xlabel("<i>x</i>"),
    Guide.ylabel("<i>y</i>"),
    white_panel))
    draw(PDF(dir * "true_orbit_position.pdf", 24cm, 24cm), vstack(p_stack))
    nothing
    # set_default_plot_size(24cm, 40cm)
    # vstack(p_stack)
end
