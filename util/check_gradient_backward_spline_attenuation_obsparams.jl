...
function twin_experiment!(outdir::String, a::Adjoint{N,L,R,K,T}, model::Model{N,L,R,T}, true_params::AbstractVector{T}, initial_lower_bounds::AbstractVector{T}, initial_upper_bounds::AbstractVector{T}, dists, trials=10) where {N,L,R,K,T<:AbstractFloat}
...
function numerical_gradient!(a::Adjoint{N,L,R,K,T}, m::Model{N,L,R}, h) where {N,L,R,K,T}
    gr = Vector{T}(L+R)
    c = orbit_cost!(a, m)
    for _i in 1:N
        a.x[_i,1] += h
...
@views function numerical_covariance!(a::Adjoint{N,L,R,K,T}, m::Model{N,L,R}, h) where {N,L,R,K,T}
...
@views function numerical_covariance2!(a::Adjoint{N,L,R,K,T}, m::Model{N,L,R}, h) where {N,L,R,K,T}
    hessian = Matrix{T}(L, L)
...
function plot_twin_experiment_result(dir, a::Adjoint{N,L,R,K}, tob, obs, stddev) where {N,L,R,K}
...
function plot_twin_experiment_result_wo_errorbar(dir, a::Adjoint{N,L,R,K}, m::Model, tob, obs) where {N,L,R,K}
    white_panel = Theme(panel_fill="white")
    p_stack = Array{Gadfly.Plot}(0)
    t = collect(0.:a.dt:a.steps*a.dt)
    for _i in 1:N
...
function plot_twin_experiment_result_wo_errorbar_observation(dir, a::Adjoint{N,L,R,K}, m::Model, tob, obs, true_params, q) where {N,L,R,K}
    white_panel = Theme(panel_fill="white")
    p_stack = Array{Gadfly.Plot}(0)
    t = collect(0.:a.dt:a.steps*a.dt)
    ...
    df_tob = DataFrame(t=t, x=[m.observation(tob[:,_j], a.r)[1] for _j in 1:a.steps], data_type="true orbit")
    # _mask = isfinite.(a.obs[_i,:,1])
    _mask = isfinite.(view(reshape(obs, N+1, :), 1, :))
    # df_obs = DataFrame(t=t[_mask], x=a.obs[_i,:,1][_mask], data_type="observed") # plot one replicate for example
    df_obs = DataFrame(t=view(repeat(t; outer=[K]), :)[_mask], x=view(reshape(obs, N+1, :), 1, :)[_mask], data_type="observed")
    df_assim = DataFrame(t=t, x=[m.observation(a.x[:,_j], a.r)[1] for _j in 1:a.steps], data_type="assimilated")
    p_stack = vcat(p_stack,
    Gadfly.plot(
    layer(df_tob, x="t", y="x", color=:data_type, Geom.line, Theme(default_color=color("blue"))),
    layer(df_assim, x="t", y="x", color=:data_type, Geom.line, Theme(default_color=color("green"))),
...
function plot_orbit(dir, a::Adjoint{N,L,R,K}, m::Model, tob, obs) where {N,L,R,K}
    white_panel = Theme(panel_fill="white")
    p_stack = Array{Gadfly.Plot}(0)
    t = collect(0.:a.dt:a.steps*a.dt)
    df_tob = DataFrame(t=t, x=[m.observation(tob[:,_j], a.r)[1] for _j in 1:a.steps+1], data_type="true orbit")
