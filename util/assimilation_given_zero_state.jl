...
@views function write_assimilation_result(dir, file, assimilation_results::AssimilationResults{M}, minimum) where {M}
    mkpath(dir)
    if isnull(assimilation_results.p)
...
function assimilation_procedure!(outdir::String, file::String, a::Adjoint{N,L,K,T}, model::Model{N,L,T}, initial_lower_bounds::AbstractVector{T}, initial_upper_bounds::AbstractVector{T}, dists, trials=10) where {N,L,K,T<:AbstractFloat}
    println(STDERR, "====================================================================================================================")
    println(STDERR, outdir)
    assimres, minres = assimilate!(a, model, initial_lower_bounds, initial_upper_bounds, dists, trials)
    if (isnull(assimres.p))
        write_assimilation_result(outdir, file, assimres, zero(T))
    else
        write_assimilation_result(outdir, file, assimres, minres.minimum)
    end
end
