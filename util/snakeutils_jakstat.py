...
def format_list_or_float3(a):
    if isinstance(a, list):
        return "_".join("{:.3f}".format(val) for val in a)
    else:
        return "{:.3f}".format(a)
...
def makepath(dirdic, config):
    ...
    return "/".join(["/".join("{}".format(val) for val in dirdic.values()), "/".join("{}_{}".format(key,format_list_or_float3(val)) for (key,val) in config.items())])
...
def make_param_path(paramdic):
    return "/".join("{}_{:.3f}".format(*item) for item in paramdic.items())
...
def format_divide_dic(basedir, paramsdic):
    ...
    """
    return ["/".join([basedir, "/".join("{}_{:.3f}".format(*i) for i in zip(paramsdic.keys(), item))]) for item in itertools.product(*paramsdic.values())]
...
def format_divide_dic_file(basedir, paramsdic, filename):
    ...
    """
    return ["/".join([basedir, "/".join("{}_{:.3f}".format(*i) for i in zip(paramsdic.keys(), item)), filename]) for item in itertools.product(*paramsdic.values())]
