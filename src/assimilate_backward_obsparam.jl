function covariance_from_θ0!(a::Adjoint{N,L,R,K,T}, m::Model{N,L,R,T}, θ0) where {N,L,R,K,T<:AbstractFloat}
    initialize!(a, θ0)
    orbit!(a, m)
...
function assimilate!(a::Adjoint{N,L,R,K,T}, m::Model, initial_lower_bounds, initial_upper_bounds, dists, trials=10) where {N,L,R,K,T<:AbstractFloat}
    mincost = Inf
