...
next_x!(        m::Model,       dt, t, x, p, x_nxt)                                                 = (m.dxdt!(    m.dxdt,     t, x, p);                                         x_nxt .=  x .+ m.dxdt                                                           .* dt; nothing)

next_dx!(       m::Model,       dt, t, x, p,       dx, dp, dx_nxt)                                  = (m.jacobian!(m.jacobian, t, x, p);                                        dx_nxt .= dx .+ m.jacobian  * CatView(dx, dp)                                    .* dt; nothing)

@views prev_λ!( m::Model{N},    dt, t, x, p,                     λ, λ_prev)            where {N}    = (m.jacobian!(m.jacobian, t, x, p);                                        λ_prev .=  λ .+ m.jacobian' *  λ[1:N]                                            .* dt; nothing)

@views prev_dλ!(m::Model{N, L}, dt, t, x, p,       dx, dp,       λ,       dλ, dλ_prev) where {N, L} = (m.hessian!( m.hessian,  t, x, p); prev_λ!(m, dt, t, x, p, dλ, dλ_prev); dλ_prev .+= reshape(reshape(m.hessian, N*L, L) * CatView(dx, dp), N, L)' * λ[1:N] .* dt; nothing)


@views function orbit!(a, m)
    for _i in 1:a.steps
        next_x!(m, a.dt, a.dt*(_i-1), a.x[:,_i], a.p, a.x[:,_i+1])
    end
    nothing
end
...
@views function neighboring!(a, m)
    for _i in 1:a.steps
        next_dx!(m, a.dt, a.dt*(_i-1), a.x[:,_i], a.p,           a.dx[:,_i], a.dp, a.dx[:,_i+1])
    end
    nothing
end
...
@views function gradient!(a::Adjoint{N}, m::Model{N}) where {N} # assuming x[:,1] .= x0; p .= p; orbit!(dxdt, t, x, p, dt); is already run. λ[:,1] is the gradient.
    ...
    for _i in a.steps:-1:1
        prev_λ!(m, a.dt, a.dt*(_i-1), a.x[:,_i], a.p,                                      a.λ[:,_i+1], a.λ[:,_i]) # fix me! a.dt*_i?
...
@views function hessian_vector_product!(a::Adjoint{N}, m::Model{N}) where {N} # assuming dx[:,1] .= dx0; dp .= dp; neighboring!(jacobian, dt, t, x, p, dx, dp); is already run. dλ[:,1] is the hessian_vector_product.
    ...
    for _i in a.steps:-1:1
        prev_dλ!(m, a.dt, a.dt*(_i-1), a.x[:,_i], a.p,           a.dx[:,_i], a.dp,             a.λ[:,_i+1],            a.dλ[:,_i+1], a.dλ[:,_i]) # fix me! a.dt*_i?
