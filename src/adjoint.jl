...
next_x!(        a::Adjoint,       t, x, x_nxt)                                               = (a.dxdt!(    a, t, x);                                  x_nxt .=  x .+ a.dxdt                                                             .* a.dt; nothing)

next_dx!(       a::Adjoint,       t, x,        dx, dx_nxt)                                   = (a.jacobian!(a, t, x);                                 dx_nxt .= dx .+ a.jacobian  * CatView(dx, a.dp)                                    .* a.dt; nothing)

@views prev_λ!( a::Adjoint{N},    t, x,                   λ, λ_prev)            where {N}    = (a.jacobian!(a, t, x);                                 λ_prev .=  λ .+ a.jacobian' *  λ[1:N]                                              .* a.dt; nothing)

@views prev_dλ!(a::Adjoint{N, L}, t, x,        dx,        λ,       dλ, dλ_prev) where {N, L} = (a.hessian!( a, t, x); prev_λ!(a, t, x, dλ, dλ_prev); dλ_prev .+= reshape(reshape(a.hessian, N*L, L) * CatView(dx, a.dp), N, L)' * λ[1:N] .* a.dt; nothing)
