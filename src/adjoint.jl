...
@views prev_λ!( a::Adjoint, m::Model{N},    t, x,                   λ, λ_prev)            where {N}    = (m.jacobian!(m.jacobian, t, x, a.p);                                    λ_prev .=  λ .+ m.jacobian' *  λ[1:N]                                              .* a.dt; nothing)

@views prev_dλ!(a::Adjoint, m::Model{N, L}, t, x,        dx,        λ,       dλ, dλ_prev) where {N, L} = (m.hessian!( m.hessian,  t, x, a.p); prev_λ!(a, m, t, x, dλ, dλ_prev); dλ_prev .+= reshape(reshape(m.hessian, N*L, L) * CatView(dx, a.dp), N, L)' * λ[1:N] .* a.dt; nothing)
...
@views function gradient!(a::Adjoint, m::Model{N}) where {N} # assuming x[:,1] .= x0; p .= p; orbit!(dxdt, t, x, p, dt); is already run. λ[:,1] is the gradient.
...
function numerical_gradient!(a::Adjoint, m::Model{N,L}, gr, h) where {N,L}
    c = orbit_cost!(a, m)
    for _i in 1:N
        a.x[_i,1] += h
...
@views function hessian_vector_product!(a::Adjoint, m::Model{N}) where {N} # assuming dx[:,1] .= dx0; dp .= dp; neighboring!(jacobian, dt, t, x, p, dx, dp); is already run. dλ[:,1] is the hessian_vector_product.
...
@views function fg!(F, ∇θ, θ, a::Adjoint, m::Model{N}) where {N}
    copy!(a.x[:,1], θ[1:N])
    copy!(a.p, θ[N+1:end])
    orbit!(a, m)
    if !(∇θ == nothing)
...
@views function minimize!(initial_θ, a::Adjoint, m::Model{N,L}) where {N,L} # Fixed. views is definitely needed for copy!
...
@views function covariance!(hessian, covariance, variance, stddev, a::Adjoint, m::Model{N,L}) where {N,L}
    fill!(a.dx[:,1], 0.)
    fill!(a.dp, 0.)
    for i in 1:N
        a.dx[i,1] = 1.
...
function numerical_hessian!(a::Adjoint, m::Model{N,L}, hessian, h) where {N,L}
