...
mutable struct Adjoint{N, L, T<:AbstractFloat, A<:AbstractVector, B<:AbstractMatrix, M<:Model}
    dt::T
    steps::Int
    obs_variance::T
    obs::B
    ...
    dλ::B
    model::M
    Adjoint{N, L, T, A, B, M}(dt::T, steps::Int, obs_variance::T, obs::AbstractMatrix{T}, x::AbstractMatrix{T}, p::AbstractVector{T}, dx::AbstractMatrix{T}, dp::AbstractVector{T}, λ::AbstractMatrix{T}, dλ::AbstractMatrix{T}, model::M) where {N,L,T,A,B,M} = new{N,L,T,A,B,M}(dt, steps, obs_variance, obs, x, p, dx, dp, λ, dλ, model)
end
...
function Adjoint(dt::T, steps::Int, obs_variance::T, obs::AbstractMatrix{T}, x::AbstractMatrix{T}, p::AbstractVector{T}, dx::AbstractMatrix{T}, dp::AbstractVector{T}, λ::AbstractMatrix{T}, dλ::AbstractMatrix{T}, model::Model) where {T<:AbstractFloat}
    xdim = size(x,1)
    θdim = xdim + length(p)
    Adjoint{xdim, θdim, T, typeof(p), typeof(x), typeof(model)}(dt, steps, obs_variance, obs, x, p, dx, dp, λ, dλ, model)
end
