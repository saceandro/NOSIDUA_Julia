...
@views function orbit_first!(a::Adjoint{N,L,R,K,T}, m) where {N,L,R,K,T<:AbstractFloat}
    # res = zeros(N)
    # x_min = zeros(N)
    # res_min = zeros(N)
    m.calc_eqdot!(m, a.p[2:end])
    for _i in 1:a.steps
...
@views function orbit!(a::Adjoint{N,L,R,K,T}, m) where {N,L,R,K,T<:AbstractFloat}
    # res = zeros(N)
    # x_min = zeros(N)
    # res_min = zeros(N)
    m.calc_eqdot!(m, a.p[2:end])
...
@views function obs_variance!(a::Adjoint{N,L,R,K}, m::Model{N,L,R}) where {N,L,R,K}
...
@views function gradient!(a::Adjoint{N,L,R,K}, m::Model{N,L,R}, gr) where {N,L,R,K} # assuming x[:,1] .= x0; p .= p; orbit!(dxdt, t, x, p, dt); is already run. λ[:,1] is the gradient.
    m.calc_eqdot!(m, a.p[2:end])
    ...
    gr[1:N]     .= a.λ[1:N,2]   .+ innovation_λ(m, a.x[:,1], a.r, a.obs_mean[1:N,1], obs_variance_over_K[1:N], a.finite[1:N,1])
    gr[N+1]     .= a.λ[N+1,2]   .+ a.regularization .* a.p[1]
    gr[N+2:L]   .= a.λ[N+2:L,2] .+ (exp.(a.p[2:end]) .- a.obs_mean[N+1,:][a.finite[N+1,:]]) ./ obs_variance_over_K[N+1] .* exp.(a.p[2:end])
...
@views function hessian_vector_product!(a::Adjoint{N,L,R,K}, m::Model{N,L,R}, x_minus_mean_obs, u_minus_mean_obs, x_minus_mean_obs_times_dx, u_minus_mean_obs_times_du, hv) where {N,L,R,K} # assuming dx[:,1] .= dx0; dp .= dp; neighboring!(jacobian, dt, t, x, p, dx, dp); is already run. dλ[:,1] is the hessian_vector_product.
...
@views function cost(a::Adjoint{N,L,R,K}) where {N,L,R,K} # assuming x[:,1] .= x0; orbit!(dxdt, t, x, p, dt); is already run
...
@views function negative_log_likelihood(a::Adjoint{N,L,R,K}, pseudo_obs, pseudo_obs_var) where {N,L,R,K} # bug! fix me!
    c = zero(a.dt)
    for _i in 1:N
        if a.Nobs[_i] > 0
...
@views function initialize!(a::Adjoint{N,L}, θ) where {N,L}
    copy!(a.x[:,1], θ[1:N])
    copy!(a.p, θ[N+1:L])
    copy!(a.r, θ[L+1:end])
    nothing
...
@views function next_x!(a::Adjoint{N,L,R,K,T},       m::Model,       t, x_prev, x) where {N,L,R,K,T}
...
@views function prev_λ!( a::Adjoint{N},    m::Model{N},    t, x,                   λ, λ_prev, obs_mean, obs_variance_over_K, finite) where {N}
    inv_j!(a, m, t, x)
    m.jacobianp!(m, t, x, a.p)
    λ_prev[1:N] .= a.jacobian_inv' * (λ[1:N] .+ innovation_λ(m, x, a.r, obs_mean[1:N], obs_variance_over_K[1:N], finite[1:N]))
...
@views function minimize!(initial_θ, a::Adjoint{N,L,R,K,T}, m::Model{N,L,R,T}) where {N,L,R,K,T<:AbstractFloat} # Fixed. views is definitely needed for copy!
...
@views function _covariance!(a::Adjoint{N,L,R,K,T}, m::Model{N,L,R,T}, x_minus_mean_obs, u_minus_mean_obs, x_minus_mean_obs_times_dx, hv) where {N,L,R,K,T<:AbstractFloat}
    neighboring!(a, m)
    for _j in 1:N
...
@views function covariance!(a::Adjoint{N,L,R,K,T}, m::Model{N,L,R,T}) where {N,L,R,K,T<:AbstractFloat}
    fill!(a.dx[:,1], 0.)
    fill!(a.dp, 0.)
