...
@views function orbit_first!(a::Adjoint{N,L,K,T}, m) where {N,L,K,T<:AbstractFloat}
    # res = zeros(N)
    # x_min = zeros(N)
    # res_min = zeros(N)
    m.calc_eqdot!(m, a.p[5:end])
    for _i in 1:a.steps
...
@views function obs_variance!(a::Adjoint{N,L,K}, m::Model{N,L}) where {N,L,K}
    ...
    a.obs_variance[N+1] = (a.pseudo_obs_TSS[N+1] + K * (mapreduce(abs2, +, exp.(a.p[5:end]) .- a.obs_mean[N+1,:][a.finite[N+1,:]]) + a.obs_filterd_var[N+1])) / a.Nobs[N+1]
    nothing
end
...
@views function gradient!(a::Adjoint{N,L,K}, m::Model{N,L}, gr) where {N,L,K} # assuming x[:,1] .= x0; p .= p; orbit!(dxdt, t, x, p, dt); is already run. λ[:,1] is the gradient.
    ...
    gr[1:N]     .= a.λ[1:N,2]   .+ innovation_λ.(m, a.x[:,1], a.obs_mean[1:N,1], obs_variance_over_K[1:N], a.finite[1:N,1])
    gr[N+1:N+4] .= a.λ[N+1:N+4,2] .+ a.regularization .* a.p[1:4]
    gr[N+5:L]   .= a.λ[N+5:L,2] .+ (exp.(a.p[5:end]) .- a.obs_mean[N+1,:][a.finite[N+1,:]]) ./ obs_variance_over_K[N+1] .* exp.(a.p[5:end])
    nothing
end

@views function hessian_vector_product!(a::Adjoint{N,L,K}, m::Model{N,L}, x_minus_mean_obs, u_minus_mean_obs, x_minus_mean_obs_times_dx, u_minus_mean_obs_times_du, hv) where {N,L,K} # assuming dx[:,1] .= dx0; dp .= dp; neighboring!(jacobian, dt, t, x, p, dx, dp); is already run. dλ[:,1] is the hessian_vector_product.
    ...
    hv[N+1:N+4] .= a.dλ[N+1:N+4,2] .+ a.regularization .* a.dp[1:4]
    hv[N+5:L]   .= a.dλ[N+5:L,2] .+ K_over_obs_variance[N+1] * ( (2. .* exp.(a.p[5:end]) .- a.obs_mean[N+1,:][a.finite[N+1,:]]) .* exp.(a.p[5:end]) .* a.dp[5:end] .- K_over_obs_variance[N+1] * oftype(K_over_obs_variance[N+1], 2.) / a.Nobs[N+1] * u_minus_mean_obs * u_minus_mean_obs_times_du)
    nothing
end
...
@views function prev_λ!( a::Adjoint{N},    m::Model{N},    t, x,                   λ, λ_prev, obs_mean, obs_variance_over_K, finite) where {N}
    inv_j!(a, m, t, x)
    m.jacobianp!(m, t, x, a.p)
    λ_prev[1:N] .= a.jacobian_inv' * (λ[1:N] .+ innovation_λ.(m, x, obs_mean[1:N], obs_variance_over_K[1:N], finite[1:N]))
...
@views function minimize!(initial_θ, a::Adjoint{N,L,K,T}, m::Model{N,L,T}) where {N,L,K,T<:AbstractFloat} # Fixed. views is definitely needed for copy!
    ...
    options = Optim.Options(;x_tol=1e-16, f_tol=1e-15, g_tol=1e-8, iterations=1_000, store_trace=true, show_trace=false, show_every=1)
...
@views function _covariance!(a::Adjoint{N,L,K,T}, m::Model{N,L,T}, x_minus_mean_obs, u_minus_mean_obs, x_minus_mean_obs_times_dx, hv) where {N,L,K,T<:AbstractFloat}
    neighboring!(a, m)
    for _j in 1:N
        x_minus_mean_obs_times_dx[_j] .= mapreduce(nanzero, +, m.d_observation.(a.x[_j,:]) .* x_minus_mean_obs[_j,:] .* a.dx[_j,:])
    end
    u_minus_mean_obs_times_du = mapreduce(nanzero, +, u_minus_mean_obs .* a.dp[5:end])
...
@views function covariance!(a::Adjoint{N,L,K,T}, m::Model{N,L,T}) where {N,L,K,T<:AbstractFloat}
    ...
    u_minus_mean_obs = (exp.(a.p[5:end]) .- a.obs_mean[N+1,:][a.finite[N+1,:]]) .* exp.(a.p[5:end])
