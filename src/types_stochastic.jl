...
#     end
#
#     Model{N, M, R, U, t, typeof(dxdt), typeof(jacobianx), typeof(hessianxx), typeof(dxdt!), typeof(jacobianx!), typeof(jacobianp!), typeof(hessianxx!), typeof(hessianxp!), typeof(hessianpp!), typeof(observation_hessianxr!), typeof(observation_hessianrr!), typeof(observation!), typeof(observation_jacobianx!), typeof(observation_jacobianr!), typeof(observation_hessianxx!)}(dxdt, jacobianx, jacobianp, hessianxx, hessianxp, hessianpp, observation, observation_jacobianx, observation_jacobianr, observation_hessianxx, observation_hessianxr, observation_hessianrr, dxdt!, jacobianx!, jacobianp!, hessianxx!, hessianxp!, hessianpp!, observation!, observation_jacobianx!, observation_jacobianr!, observation_hessianxx!, observation_hessianxr!, observation_hessianrr!, time_point, Δtime_point)
# end


mutable struct Model{N, M, R, U, T<:AbstractFloat, A<:AbstractVector, B<:AbstractMatrix, C<:AbstractArray} #<: AbstractModel{N, L, T, A, B, C, F, G, H}
    dxdt::A
    jacobianx::B
    jacobianp::B
    hessianxx::C
    ...
    hessianpp!
    observation!
    observation_jacobianx!
    observation_jacobianr!
    observation_hessianxx!
    observation_hessianxr!
    observation_hessianrr!
    time_point::A
    Δtime_point::A
    Model{N, M, R, U, T, A, B, C}(dxdt::AbstractVector{T}, jacobianx::AbstractMatrix{T}, jacobianp::AbstractMatrix{T}, hessianxx::AbstractArray{T,3}, hessianxp::AbstractArray{T,3}, hessianpp::AbstractArray{T,3}, observation::AbstractVector{T}, observation_jacobianx::AbstractMatrix{T}, observation_jacobianr::AbstractMatrix{T}, observation_hessianxx::AbstractArray{T,3}, observation_hessianxr::AbstractArray{T,3}, observation_hessianrr::AbstractArray{T,3}, dxdt!, jacobianx!, jacobianp!, hessianxx!, hessianxp!, hessianpp!, observation!, observation_jacobianx!, observation_jacobianr!, observation_hessianxx!, observation_hessianxr!, observation_hessianrr!, time_point::AbstractVector{T}, Δtime_point::AbstractVector{T}) where {N,M,R,U,T,A,B,C} = new{N,M,R,U,T,A,B,C}(dxdt, jacobianx, jacobianp, hessianxx, hessianxp, hessianpp, observation, observation_jacobianx, observation_jacobianr, observation_hessianxx, observation_hessianxr, observation_hessianrr, dxdt!, jacobianx!, jacobianp!, hessianxx!, hessianxp!, hessianpp!, observation!, observation_jacobianx!, observation_jacobianr!, observation_hessianxx!, observation_hessianxr!, observation_hessianrr!, time_point, Δtime_point)
end

function Model(t::Type{T}, N, M, R, U, time_point::AbstractVector{T}, dxdt!, jacobianx!, jacobianp!, hessianxx!, hessianxp!, hessianpp!, observation!, observation_jacobianx!, observation_jacobianr!, observation_hessianxx!, observation_hessianxr!, observation_hessianrr!) where {T<:AbstractFloat}
    dxdt = Array{t}(undef, N)
    jacobianx = zeros(t, N, N)
    jacobianp = zeros(t, N, M)
    hessianxx = zeros(t, N, N, N)
    ...
    Model{N, M, R, U, t, typeof(dxdt), typeof(jacobianx), typeof(hessianxx)}(dxdt, jacobianx, jacobianp, hessianxx, hessianxp, hessianpp, observation, observation_jacobianx, observation_jacobianr, observation_hessianxx, observation_hessianxr, observation_hessianrr, dxdt!, jacobianx!, jacobianp!, hessianxx!, hessianxp!, hessianpp!, observation!, observation_jacobianx!, observation_jacobianr!, observation_hessianxx!, observation_hessianxr!, observation_hessianrr!, time_point, Δtime_point)
end
...
mutable struct Adjoint{N, M, R, U, K, T<:AbstractFloat, A<:AbstractVector, B<:AbstractMatrix, D<:AbstractVector, E<:AbstractMatrix}
    ...
    jacobian_inv::B
    regularization::T
    Adjoint{N, M, R, U, K, T, A, B, D, E}(dt::T, steps::Int, t::AbstractVector{T}, sys_precision::AbstractMatrix{T}, obs_precision::AbstractMatrix{T}, obs_mean::AbstractMatrix{T}, obs_filterd_var::AbstractVector{T}, finite::AbstractMatrix{Bool}, Nobs::AbstractVector{Int}, pseudo_obs_TSS::AbstractVector{T}, x::AbstractMatrix{T}, p::AbstractVector{T}, r::AbstractVector{T}, dx::AbstractMatrix{T}, dp::AbstractVector{T}, dr::AbstractVector{T}, λ::AbstractMatrix{T}, dλ::AbstractMatrix{T}, res::AbstractVector{T}, jacobian_inv::AbstractMatrix{T}, regularization::T) where {N,M,R,U,K,T,A,B,D,E} = new{N,M,R,U,K,T,A,B,D,E}(dt, steps, t, sys_precision, obs_precision, obs_mean, obs_filterd_var, finite, Nobs, pseudo_obs_TSS, x, p, r, dx, dp, dr, λ, dλ, res, jacobian_inv, regularization)
end

function Adjoint(dt::T, total_T::T, sys_variance::AbstractVector{T}, obs_variance::AbstractVector{T}, pseudo_Nobs::AbstractVector{Int}, pseudo_obs_var::AbstractVector{T}, x0::AbstractVector{T}, p::AbstractVector{T}, r::AbstractVector{T}, replicates::Int, regularization::T) where {T<:AbstractFloat}
    ...
    sys_precision = diagm(0 => inv.(sys_variance))
    obs_precision = diagm(0 => inv.(obs_variance))
    ...
    Adjoint{xdim, pdim, rdim, obsdim, replicates, T, typeof(p), typeof(x), typeof(Nobs), typeof(finite)}(dt, steps-1, t, sys_precision, obs_precision, obs_mean, obs_filterd_var, finite, Nobs, pseudo_obs_TSS, x, p, r, dx, dp, dr, λ, dλ, res, jacobian, regularization) # fixed bug. K=replicates.
end
