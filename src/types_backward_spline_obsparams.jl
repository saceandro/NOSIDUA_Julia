mutable struct Model{N, L, R, T<:AbstractFloat, A<:AbstractVector, B<:AbstractMatrix, C<:AbstractArray, F<:Function, G1<:Function, G2<:Function, H1<:Function, H2<:Function, H3<:Function, O<:Function, P<:Function, I<:Function, J<:Function, K<:Function, M<:Function, S<:LinAlg.LU} #<: AbstractModel{N, L, T, A, B, C, F, G, H}
    dxdt::A
    jacobianx::B
    jacobianp::B
    hessianxx::C
    ...
    Model{N, L, R, T, A, B, C, F, G1, G2, H1, H2, H3, O, P, I, J, K, M, S}(dxdt::AbstractVector{T}, jacobianx::AbstractMatrix{T}, jacobianp::AbstractMatrix{T}, hessianxx::AbstractArray{T,3}, hessianxp::AbstractArray{T,3}, hessianpp::AbstractArray{T,3}, dxdt!::F, jacobianx!::G1, jacobianp!::G2, hessianxx!::H1, hessianxp!::H2, hessianpp!::H3, calc_eqdot!::O, observation::I, d_observation::J, dd_observation::K, inv_observation::M, dr_observation::AbstractMatrix{T}, dr_observation!::P, time_point::AbstractVector{T}, Δtime_point::AbstractVector{T}, eqdot::AbstractVector{T}, rhs::AbstractVector{T}, spline_lhs::LinAlg.LU{T,Tridiagonal{T}}, spline_rhs::AbstractMatrix{T}, spline_lhs_inv::AbstractMatrix{T}, spline_lhs_inv_rhs::AbstractMatrix{T}) where {N,L,R,T,A,B,C,F,G1,G2,H1,H2,H3,O,P,I,J,K,M,S} = new{N,L,R,T,A,B,C,F,G1,G2,H1,H2,H3,O,P,I,J,K,M,S}(dxdt, jacobianx, jacobianp, hessianxx, hessianxp, hessianpp, dxdt!, jacobianx!, jacobianp!, hessianxx!, hessianxp!, hessianpp!, calc_eqdot!, observation, d_observation, dd_observation, inv_observation, dr_observation, dr_observation!, time_point, Δtime_point, eqdot, rhs, spline_lhs, spline_rhs, spline_lhs_inv, spline_lhs_inv_rhs)
end

function Model(t::Type{T}, N, L, R, time_point::AbstractVector{T}, dxdt!::F, jacobianx!::G1, jacobianp!::G2, hessianxx!::H1, hessianxp!::H2, hessianpp!::H3, calc_eqdot!::O, observation::I, d_observation::J, dd_observation::K, inv_observation::M, dr_observation!::P) where {T<:AbstractFloat, F<:Function, G1<:Function, G2<:Function, H1<:Function, H2<:Function, H3<:Function, O<:Function, P<:Function, I<:Function, J<:Function, K<:Function, M<:Function}
    _M = L-N
    dxdt = Array{t}(N)
    ...
    Model{N, L, R, t, typeof(dxdt), typeof(jacobianx), typeof(hessianxx), typeof(dxdt!), typeof(jacobianx!), typeof(jacobianp!), typeof(hessianxx!), typeof(hessianxp!), typeof(hessianpp!), typeof(calc_eqdot!), typeof(dr_observation!), typeof(observation), typeof(d_observation), typeof(dd_observation), typeof(inv_observation), typeof(spline_lhs)}(dxdt, jacobianx, jacobianp, hessianxx, hessianxp, hessianpp, dxdt!, jacobianx!, jacobianp!, hessianxx!, hessianxp!, hessianpp!, calc_eqdot!, observation, d_observation, dd_observation, inv_observation, dr_observation, dr_observation!, time_point, Δtime_point, eqdot, rhs, spline_lhs, spline_rhs, spline_lhs_inv, spline_lhs_inv_rhs)
end
...
mutable struct Adjoint{N, L, R, K, T<:AbstractFloat, A<:AbstractVector, B<:AbstractMatrix, D<:AbstractVector, E<:AbstractMatrix}
    dt::T
    steps::Int
    t::A
    obs_variance::A
    ...
    newton_tol::T
    regularization::T
    Adjoint{N, L, R, K, T, A, B, D, E}(dt::T, steps::Int, t::AbstractVector{T}, obs_variance::AbstractVector{T}, obs_mean::AbstractMatrix{T}, obs_filterd_var::AbstractVector{T}, finite::AbstractMatrix{Bool}, Nobs::AbstractVector{Int}, pseudo_obs_TSS::AbstractVector{T}, x::AbstractMatrix{T}, p::AbstractVector{T}, r::AbstractVector{T}, dx::AbstractMatrix{T}, dp::AbstractVector{T}, λ::AbstractMatrix{T}, dλ::AbstractMatrix{T}, res::AbstractVector{T}, jacobian_inv::AbstractMatrix{T}, trylimit::Int, newton_tol::T, regularization::T) where {N,L,R,K,T,A,B,D,E} = new{N,L,R,K,T,A,B,D,E}(dt, steps, t, obs_variance, obs_mean, obs_filterd_var, finite, Nobs, pseudo_obs_TSS, x, p, r, dx, dp, λ, dλ, res, jacobian_inv, trylimit, newton_tol, regularization)
end
...
function Adjoint(dt::T, total_T::T, pseudo_Nobs::AbstractVector{Int}, pseudo_obs_var::AbstractVector{T}, x0::AbstractVector{T}, p::AbstractVector{T}, r::AbstractVector{T}, replicates::Int, trylimit::Int, newton_tol::T, regularization::T) where {T<:AbstractFloat}
    ...
    rdim = length(r)
    t = collect(0.:dt:dt*(steps-1))
    obs_variance = Vector{T}(xdim+1)
    Nobs = copy(pseudo_Nobs)
    pseudo_obs_TSS = pseudo_Nobs .* pseudo_obs_var
    finite = Matrix{Bool}(xdim+1, steps)
    obs_mean = Matrix{T}(xdim+1, steps)
    obs_filterd_var = Vector{T}(xdim+1)
    x = similar(x0, xdim, steps)
    @views copy!(x[:,1], x0)
    dx = similar(x0, xdim, steps)
    dp = similar(p)
    ...
    res = zeros(T, xdim)
    jacobian = similar(x0, xdim, xdim)
    Adjoint{xdim, θdim, rdim, replicates, T, typeof(p), typeof(x), typeof(Nobs), typeof(finite)}(dt, steps-1, t, obs_variance, obs_mean, obs_filterd_var, finite, Nobs, pseudo_obs_TSS, x, p, r, dx, dp, λ, dλ, res, jacobian, trylimit, newton_tol, regularization) # fixed bug. K=replicates.
end

function Adjoint(Nparams::Int, Nobsparams::Int, dt::T, obs::AbstractArray{T,3}, pseudo_Nobs::AbstractVector{Int}, pseudo_obs_var::AbstractVector{T}, x0::AbstractVector{T}, trylimit::Int, newton_tol::T, regularization::T) where {T<:AbstractFloat}
    xdim, steps, replicates = size(obs)
    θdim = xdim + Nparams
    rdim = Nobsparams
    t = collect(0.:dt:dt*(steps-1))
    ...
    all!(finite, isfinite.(obs))
    obs_mean = reshape(mean(obs, 3), xdim+1, steps)
    for _j in 1:xdim+1
        obs_filterd_var[_j] = mapreduce(x -> isnan(x) ? zero(x) : x, +, var(obs[_j,:,:], 2; corrected=false))
        Nobs[_j] .+= count(isfinite.(obs[_j,:,:]))
    end
