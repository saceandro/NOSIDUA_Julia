...
@views function next_dx!(a::Adjoint,       m::Model,       t, x,        dx, dx_nxt)
    ...
    dx_nxt2 = (a.dx[1,1] - a.dt * a.x[1,2] * a.dp[1]) / (1. + a.dt * a.p[1])
    println("dx_nxt: $(dx_nxt)\tdx_nxt2: $(dx_nxt2)")
    # dx_nxt[1] = dx_nxt2
    # dx_nxt .= (dx .- a.dt * x .* a.dp) ./ (1. + a.dt * a.dp)
    # println("jacobian_inv: $(a.jacobian_inv)\t1/(1. + a.dt*a.dp): $(1./(1. + a.dt*a.dp))")
    nothing
end
...
@views function prev_λ!( a::Adjoint{N},    m::Model{N},    t, x,                   λ, λ_prev, obs_mean, obs_variance_over_K, finite) where {N}
    ...
    #
    # λ_prev_x = (x[1] - obs_mean[1])/obs_variance_over_K[1]/(1. + a.dt * a.p)
    # λ_prev_p = -a.dt * λ_prev_x * x[1]
    #
    # println("λ_prev[1:N]: $(λ_prev[1:N])\tλ_prev_x: $λ_prev_x")
    nothing
end
...
@views function prev_dλ!(a::Adjoint{N, L}, m::Model{N, L}, t, x,        dx,        λ,       dλ, dλ_prev, Nobs, K_over_obs_variance, x_minus_mean_obs, x_minus_mean_obs_times_dx, finite) where {N, L}
    ...
    # dλ_prev[N+1:end] .= ( reshape( reshape(permutedims(m.hessianxp, [1,3,2]), N*M, N) * dx, N, M )' * λ[1:N] .+ m.jacobianp' * dλ_prev[1:N] ) .* a.dt # bug fixed

    dλ_prev_x = (-a.dt * a.λ[1,2] * a.dp[1] + K_over_obs_variance[1] * (a.dx[1,2] - K_over_obs_variance[1] * x_minus_mean_obs[1] * x_minus_mean_obs_times_dx[1] ) ) / (1. + a.dt * a.p[1])
    dλ_prev_p = -a.dt * (a.x[1,2] * a.dλ[1,2] + a.λ[1,2] * a.dx[1,2])
    # # dλ_prev_p = -a.dt * (x[1] * dλ[1] + λ[1] * dx[1])
    # dλ_prev[1] = dλ_prev_x

    println("a.steps: $(a.steps)")
    println("a.Nobs: $(a.Nobs)")

    # dλ_prev[1] と dλ_prev_xの間に10^-5オーダーのずれ。やや怪しい
    println("dλ_prev[1]: $(dλ_prev[1])\tdλ_prev_x: $dλ_prev_x")
    println("dλ_prev[2]: $(dλ_prev[2])\tdλ_prev_p: $dλ_prev_p")

    println("innovation_dλ: $((innovation_dλ.(m, x, dx, Nobs, K_over_obs_variance, x_minus_mean_obs, x_minus_mean_obs_times_dx, finite))[1])\t$(K_over_obs_variance[1] * (a.dx[1,2] - K_over_obs_variance[1] * x_minus_mean_obs[1] * x_minus_mean_obs_times_dx[1] ))")

    # println("(m.jacobianp' * dλ_prev[1:N] .* a.dt)[1]: $((m.jacobianp' * dλ_prev[1:N] .* a.dt)[1])\t-a.dt * a.x[1,2] * a.dλ[1,2]: $(-a.dt * a.x[1,2] * a.dλ[1,2])")
    # println("(reshape( reshape(permutedims(m.hessianxp, [1,3,2]), N*M, N) * dx, N, M )' * λ[1:N] .* a.dt)[1]: $((reshape( reshape(permutedims(m.hessianxp, [1,3,2]), N*M, N) * dx, N, M )' * λ[1:N] .* a.dt)[1])\t-a.dt * a.λ[1,2] * a.dx[1,2]: $(-a.dt * a.λ[1,2] * a.dx[1,2])")
